...
{
    public eclCom $component;
    public array $slot;
    public array $scopes = [];

    public function __construct(eclCom $component, array $slot = [])
    {
    ...
    public function getProperty(string $path, bool $returnString = false)
    {
        ...
        foreach ($this->scopes as $scope) {
            ...
            if (preg_match('/^[A-Z][a-zA-Z0-9_]*$/', $name)) {
                ...
                $current = $scopeName::getScope($this->component->page, $current, array_shift($parts) ?? '');
                continue;
            }
